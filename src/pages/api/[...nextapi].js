        ...
        case 'POST':
          ...
          case 'PUT':
            if (pathname.startsWith('/api/aproducts/')) {
              ...
              const id = pathname.split('/').pop();
              return authMiddleware((req, res) => handleUpdateProductPrice(req, res, id))(req, res);
...
async function handleDeleteProduct(req, res, id) {
  try {
    // First delete associated prices due to foreign key constraint
    await db.query('DELETE FROM aproduct_prices WHERE productid = ?', [id]);
    // Then delete the product
    await db.query('DELETE FROM aproducts WHERE productid = ?', [id]);
    res.status(200).json({ message: 'Product and associated prices deleted successfully' });
  } catch (error) {
